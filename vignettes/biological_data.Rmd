...
this vignette deals with its use to visualise gene expression patterns and sample relationships in RNAseq data relating to breast cancer samples, as retrieved from from [Genomic Data Commons].
...
This function was used in the present example to provide normalised expression 
values. While pre-processing the data, quantile and TMM normalisation
...
the total distance between values. Furthermore,
it is typically the difference between samples which is of interest, rather
than the difference between measures. Non-centred data may show that all
samples measure high for one variable, and low for another, while centred
data shows relative differences. Alternatively, one could use a distance measure
...
is observed in normalised data. Samples appear to cluster based 
